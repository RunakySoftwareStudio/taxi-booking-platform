...
async function addAvailability(formData: FormData) {
  ...
  const { error } = await supabaseAdmin.from("chauffeur_availability").insert({
...
export default async function ChauffeurAvailabilityPage({
  params,
}: ChauffeurAvailabilityPageProps) {
  ...
  const { data: availabilityStatuses, error: availabilityStatusError } =
  ...
  if (availabilityError) {
    console.error("Could not load availability:", availabilityError);
  }

  if (availabilityStatusError) {
    console.error("Could not load availability statuses:", availabilityStatusError);
  }
    ...
    <main className="min-h-screen bg-slate-950 px-6 py-16 text-white">
      <div className="mx-auto max-w-6xl">
        ...
        <h1 className="mt-3 text-3xl font-bold">
          Availability for {chauffeurRow.name}
        </h1>

        <p className="mt-4 max-w-2xl text-slate-300">
          Add the times when you are available, busy, offline, or on holiday.
        </p>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-4">
            <label className="block">
              ...
              <input
                ...
                type="date"
                required
                className="w-full rounded-lg border border-white/10 bg-slate-950 px-4 py-3 text-white"
              />
            </label>
