...
CREATE TYPE taxi_operator_verification_status AS ENUM (
...
CREATE TABLE taxi_operators (
  ...
  verification_status taxi_operator_verification_status
...
ON taxi_operators(verification_status);

CREATE INDEX taxi_operators_company_name_idx
ON taxi_operators(company_name);
...
CREATE TABLE chauffeurs (
  ...
  phone TEXT NOT NULL,
  /* Taxi business responsible for this chauffeur. */
  operator_id UUID REFERENCES taxi_operators(id),
  company_name TEXT,
  license_number TEXT,
  service_area TEXT,
...
/* Find all chauffeurs belonging to one taxi operator. */
CREATE INDEX chauffeurs_operator_id_idx ON chauffeurs(operator_id);
...
/* Apply updated_at trigger to taxi operators */
CREATE TRIGGER update_taxi_operators_updated_at
BEFORE UPDATE ON taxi_operators
FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();
...
/* ============================================================================================================================
   End PRICING MARKET WEEKLY SCHEDULE VALIDATION + ATOMIC REPLACEMENT
=============================================================================================================================== */
...
/* ============================================================================================================================
   ...
   - The database is the final safety layer.
    ...
    So the readiness flow is now:
        mark_pricing_market_ready('DE')
            ↓
        checks market state
                ↓
        checks 5 approved pricing profiles + rates
                ↓
        calls:
        validate_pricing_market_weekly_schedule('DE')
                ↓
        database verifies:
        ✓ all 7 days
        ✓ starts at 00:00
        ...
        ✓ no gaps
        ✓ no overlaps
        ✓ profile families belong to market
                ↓
        checks tax
                ↓
        checks rounding
...
BEGIN
    ...
    /* ===== Validate complete weekly schedule ===== */
    PERFORM public.validate_pricing_market_weekly_schedule(v_country_code);
...
IS 'Marks a review-required pricing market ready only after validating five active pricing profiles with rates, a complete valid seven-day weekly pricing schedule, one approved terminal tax rule and one approved terminal currency rounding rule. Pricing remains disabled.';
/* ============================================================================================================================
   End MARK PRICING MARKET READY
=============================================================================================================================== */
...
/* =====================================================================================================================
   ...
   - When a document is verified, other verified documents
     of the same type become superseded.
   - Chauffeur account/compliance/operational status is unchanged.
======================================================================================================================= */
